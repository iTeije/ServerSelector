...
public class HelpSubCommand extends SubCommand {
    ...
    public void onExecute(CommonExecutor sender, String[] args, String label) {
        ...
        if (args.length == 1) {
            ...
            if (subCommand != null) {
                spigotMessageModule.sendToPlayer(StorageKey.HELP_COMMAND_DEDICATED, new Player[]{player}, MessageType.SPIGOT_MESSAGE,
                        new Replacement("{command}", commandModule.getFullCommand(label, args), ReplacementType.COMMAND)
                        );
                if (subCommand.getArguments().size() == 0) {
                    spigotMessageModule.sendToPlayer(StorageKey.HELP_COMMAND_NO_RESULTS, new Player[]{player}, MessageType.SPIGOT_MESSAGE);
                    return;
                }
                for (Argument argument : subCommand.getArguments()) {
                    spigotMessageModule.sendToPlayer(StorageKey.HELP_COMMAND_ITEM, new Player[]{player}, MessageType.SPIGOT_MESSAGE,
                            ...
                            new Replacement("{description}", argument.getSyntax(), ReplacementType.COMMAND_DESCRIPTION)
                    );
                }
                return;
            }
        ...
        spigotMessageModule.sendToPlayer(StorageKey.HELP_COMMAND_TITLE, new Player[]{player}, MessageType.SPIGOT_MESSAGE);
